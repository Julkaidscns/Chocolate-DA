...
parameters {
  ...
  vector[J] alpha_raw;          // non-centered
  vector[K] gamma_raw;
}
...
model {
  // Priors — each variable gets exactly one prior
  mu_global      ~ normal(0, 1);
  sigma_origin   ~ normal(0, 1);   // HalfNormal(1) due to <lower=0>
  sigma_producer ~ normal(0, 1);
  
  alpha_raw ~ std_normal();        // REQUIRED for non-centered param
  gamma_raw ~ std_normal();
  
  c ~ normal(0, 2.5);             // slightly wider helps ordered constraint
